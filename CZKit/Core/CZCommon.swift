...
public var cz_statusBarHeight: CGFloat {
    if #available(iOS 13.0, *) {
        return (CZCommon.lastWindow()?.windowScene?.statusBarManager?.statusBarFrame.height)!
    } else {
...
public class CZCommon: NSObject {
    ...
    /// 删除所有偏好设置
    static func removeAllUserDefaults() -> Void {
        UserDefaults.standard.removePersistentDomain(forName: Bundle.main.bundleIdentifier!)
    }

    /// 跳转到AppStore进行评分
    /// - Parameter appId: appId
    static func jumpAppStoreScore(_ appId: String) {
    ...
    /// 跳转到AppStore首页
    /// - Parameter appId: appId
    static func jumpAppStotrScore(_ appId: String) {
    ...
    static func callSystemDial(phoneNumber: String) {
    ...
    /// 获取最上层的控制器
    static func topController(_ viewController: UIViewController = (UIApplication.shared.delegate?.window?!.rootViewController)!) -> UIViewController {
        if viewController.isKind(of: UINavigationController.self) {
            return topController((viewController as! UINavigationController).visibleViewController!)
        } else if viewController.isKind(of: UITabBarController.self) {
            return topController((viewController as! UITabBarController).selectedViewController!)
        } else {
            if (viewController.presentedViewController != nil) {
                return topController(viewController.presentedViewController!)
            } else {
