...
//  letaoshijie
//
//  Created by udream3 on 2021/4/20.
//
/* 枚举类型
...
public struct CZRegularManage {
    ...
    /// 获取所有的匹配结果
    public func matches(string: String, options: NSRegularExpression.MatchingOptions = []) -> [NSTextCheckingResult] {
        return regularExpression.matches(in: string, options: options, range: NSRange(location: 0, length: string.count))
    }
    
    /// 获取匹配数
    public func numberOfMatches(string: String, options: NSRegularExpression.MatchingOptions = []) -> Int {
    ...
    ///   - value: 替换值
    /// - Returns: 替换后的值
    public func replacingMatches(string: String, options: NSRegularExpression.MatchingOptions = [], value: String) -> String {
